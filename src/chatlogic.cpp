...
// DEBUGGING
int GraphNode::counter = 0;
int GraphEdge::counter = 0;

ChatLogic::ChatLogic()
{
    Print("ChatLogic Constructor");
    //// STUDENT CODE
    ////
...
ChatLogic::~ChatLogic()
{
    Print("ChatLogic Destructor");

    //// STUDENT CODE
    ////

    // delete chatbot instance              TASK 5 
    // if(_chatBot != nullptr)
    //    delete _chatBot;

    // TASK 3 - DELETE NOT NEEDED (THEY ARE SMART POINTERS)
    // delete all nodes
    ...
    // delete all edges
    // TASK 4 - THEY ARE OWNED BY EACH GraphNode, we don't need to delete them (they are unique)
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // https://knowledge.udacity.com/questions/76200
                            //                                                 Is this a Lambda ??  .
                            ...
                            edge->SetChildNode(childNode->get());           // TASK 3 - JUST INFO, 
                            edge->SetParentNode(parentNode->get());         // TASK 3 - JUST INFO, 
    ...
    // TASK 5 
    ChatBot chatbot_var("../images/chatbot.png");

    // add chatbot to graph root node
    //_chatBot->SetRootNode(rootNode);
    chatbot_var.SetRootNode(rootNode);
    chatbot_var.SetChatLogicHandle(this);
    //rootNode->MoveChatbotHere(_chatBot);
    rootNode->MoveChatbotHere(chatbot_var);
    ////
    //// EOF STUDENT CODE
}
